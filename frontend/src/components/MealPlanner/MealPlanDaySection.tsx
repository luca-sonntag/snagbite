...
export const MealPlanDaySection = React.memo<MealPlanDaySectionProps>(({
  ...
  return (
    ...
    >
      ...
      {isPast ? (
        <div
          className={`w-full min-h-[44px] flex items-center justify-between px-3.5 py-2.5 rounded-2xl select-none border-none shadow-2xs ${
            isHighlighted
              ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/25'
              : 'bg-gray-100/70 dark:bg-gray-800/60 text-gray-500 dark:text-gray-400'
          }`}
        >
          <span
            className={`text-xs sm:text-sm font-extrabold tracking-tight truncate ${
              isHighlighted ? 'text-white' : 'text-gray-500 dark:text-gray-400'
            }`}
          >
            {formattedDate}
          </span>
