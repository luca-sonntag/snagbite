          ...
          {ALL_BADGE_KEYS.map((key) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[10.5px] font-bold leading-tight text-center line-clamp-2 whitespace-normal px-0.5 ${isEarned ? 'text-emerald-950 dark:text-emerald-100' : 'text-gray-500 dark:text-gray-400'}`}>
                  {t(`app.gamification.badges.${key}`)}
                </span>

                {!isEarned && progress && progress.total > 1 && (
                  <span className="text-[9px] font-extrabold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 dark:bg-emerald-500/20 px-1.5 py-0.5 rounded-full leading-none shrink-0 mt-0.5">
