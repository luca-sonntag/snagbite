...
/**
 * Show a Rewarded Video / Rewarded Interstitial Ad.
 * Prefers the 5-second skippable Rewarded Interstitial format for a fast, user-friendly experience.
 * Falls back seamlessly to classic Rewarded Video if Rewarded Interstitial is unavailable or fails to load.
 *
 * Returns a promise that resolves to `true` if the user successfully completed
 * watching the ad (or reached the 5s reward grant) and earned the reward,
 * or `false` if dismissed early/failed/cancelled.
 *
...
async function showRewardVideoAdInternal(): Promise<boolean> {
  return new Promise<boolean>(async (resolve) => {
    ...
    const EVENT_REWARDED =
      typeof RewardAdPluginEvents !== 'undefined'
    ...
    try {
      rewardedHandle = await AdMob.addListener(EVENT_REWARDED as any, (reward: unknown) => {
        console.log('[AdMob] user earned reward from video:', reward);
        earned = true;
      });

      dismissedHandle = await AdMob.addListener(EVENT_DISMISSED as any, () => {
        console.log('[AdMob] rewarded video ad dismissed. earned =', earned);
        cleanup();
        resolve(earned);
      });

      failedHandle = await AdMob.addListener(EVENT_FAILED_TO_SHOW as any, (err: unknown) => {
        console.warn('[AdMob] rewarded video ad failed to show:', err);
        cleanup();
        resolve(false);
      });
