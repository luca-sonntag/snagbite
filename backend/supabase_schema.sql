...
INSERT INTO public.global_settings (key, value, description) VALUES
  ...
  ('rewarded_ad_bonus_credits', '3', 'Number of recipe extractions granted when watching a rewarded video ad')
...
-- Phase C: Add llm_usage JSONB column for clean separation of AI/LLM metrics (Gemini tokens/costs & FLUX cover generation) from the recipe JSON
...
-- --- AI recipe cover images (FLUX.1 schnell) ---

-- Public bucket for AI-generated recipe cover images (food photography).
-- Generated by the worker upon recipe extraction/remix, uploaded via service role.
INSERT INTO storage.buckets (id, name, public)
