...
* **Trigger (echter One-Shot):** Der Versuch wird in `App.tsx` genau **einmal pro App-Session** beim ersten „ready"-Zeitpunkt (nach Onboarding) *verbraucht*, ~1,2 s nach App-Bereitschaft, damit der Splash/First-Paint nicht überdeckt wird. Nur für Free-User.
...
* **Cold-Start-Kadenz:** Die App-Open-Ad wird nur bei jedem `APP_OPEN_SHOW_EVERY_N_COLD_STARTS`-ten (aktuell **jedem 3.**) förderfähigen Cold-Start gezeigt statt über einen Wall-Clock-Timer, persistiert als Zähler über `snagbite:appOpenAd:coldStartCount`. Der Zähler wird bereits beim Versuch hochgezählt (bzw. bei Anzeige auf 0 zurückgesetzt), sodass auch No-Fills die Kadenz weiterzählen und ein schneller Relaunch gezählt statt erneut gefeuert wird.
